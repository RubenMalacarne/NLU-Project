...
def read_file(path, eos_token="<eos>"):
    '''
        reads line by line and add token of 
        end of phrase EOS (end of sentence)
    '''
...
def get_vocab(corpus, special_tokens=[]):
    '''
        crea un vocabolario da un corpus di testi, 
        assegnando un indice numerico unico a ogni parola.
    '''
    vocab = {}
    i = 0
...
'''
    get_dataset_raw and get_raw_dataset 
    read and prepare Penn Treebank (PTB) 
...
def collate_fn(data, pad_token):
    '''
        used as the collate_fn parameter when creating a DataLoader 
        in PyTorch to manipulate and prepare data before 
        it is sent to the model for training or evaluation.
    '''
    def merge(sequences):
        '''
